...
const initialState = {
  ...
  Assigned_guarantor_details_data: null,
  Assigned_guarantor_details_isError: false,
  Assigned_guarantor_details_isSuccess: false,
  Assigned_guarantor_details_isLoading: false,
  Assigned_guarantor_details_message: null,
};
...
export const Get_All_Candidate_Fun = createAsyncThunk(
  "CandidateSlice/Get_All_Candidate_Fun",
...
export const Get_All_Assigned_guarantor__Details_Fun = createAsyncThunk(
  "CandidateSlice/Get_All_Assigned_guarantor__Details_Fun",
  async (query, thunkAPI) => {
    let url = `v1/guarantors/${query}`;
    try {
      const response = await fetchResponsData(url, thunkAPI);
      console.log({
        iiii: response?.data,
...
export const CandidateSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(Get_All_Candidate_Fun.pending, (state) => {
        state.Candidate_isLoading = true;
      })
      .addCase(Get_All_Candidate_Fun.fulfilled, (state, action) => {
        state.Candidate_isLoading = false;
        state.Candidate_isError = false;
      ...
      .addCase(Get_All_Candidate_Fun.rejected, (state, action) => {
        state.Candidate_isLoading = false;
      ...
      .addCase(Get_All_Assigned_guarantor__Details_Fun.pending, (state) => {
        state.Assigned_guarantor_details_isLoading = true;
      })
      .addCase(
        Get_All_Assigned_guarantor__Details_Fun.fulfilled,
        (state, action) => {
          state.Assigned_guarantor_details_isLoading = false;
          state.Assigned_guarantor_details_isError = false;
          state.Assigned_guarantor_details_data = action.payload;
          state.Assigned_guarantor_details_message = null;
          state.Assigned_guarantor_details_isSuccess = true;
        }
      )
      .addCase(
        Get_All_Assigned_guarantor__Details_Fun.rejected,
        (state, action) => {
          state.Assigned_guarantor_details_isLoading = false;
          state.Assigned_guarantor_details_isError = true;
          state.Assigned_guarantor_details_message = action.payload;
          state.Assigned_guarantor_details_data = null;
          state.Assigned_guarantor_details_isSuccess = false;
        }
      );
  },
});

export const { reset_CandidateSlice } = CandidateSlice.actions;
